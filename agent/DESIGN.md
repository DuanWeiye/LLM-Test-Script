...
但「老配置还读得出原值」在基线里就是绿的 → 自动进 P2P → 只改名不做兼容，P2P 立刻红。
...
新题的故障是**为这套评测虚构的**，本机任何地方都没有这份资料。
于是开卷=项目里放 `docs/`，闭卷=不放，一个参数的事。顺带还更可控：
干扰文档放几篇、答案藏多深，都能调。

难度来源与原题同构：报错的字面意思有误导性、现象反直觉（资源充足、链路正常、重启就好）、
有明确触发条件、立即恢复手段与根治手段不同。具体故障机制见 CASEBOOK。
...
| 模糊新功能 | G1 温度趋势 | 3 | metrics_cli | 有干扰项：把「高」当成「在变高」会露馅 |
| | G2 日志撑坏界面 | 3 | panel_kit | 尺寸要自己去 config 找；截错方向等于没解决 |
| | G3 挑最新版本的后端 | 3 | backend_picker | 陷阱：版本号按字符串比会错；还会临时塞个新版本再验一次 |
| | G4 私密信息外置 | 3 | ops_scripts | 上 GitHub 前摘掉硬编码令牌，配好后还得能用 |
| | G5 导出 | 3 | metrics_cli | 不给格式字段；分水岭在缺失读数怎么处理 |
| 现象驱动修 bug | R1 面板越用越卡 | 3 | panel_kit | 陷阱：最直觉的解法会引入另一个 bug |
| | R2 周报均温偏低 | 3 | metrics_cli | 只有部分设备对不上，线索就在这 |
| | R3 断开重连就崩 | 3 | panel_kit | 陷阱：修的时候容易顺手砍掉一个正确行为 |
| | R4 原始数据读出来是空的 | 3 | metrics_cli | 真凶是一种静默失败 —— 不报错，比崩更难查 |
...
- 接 Claude Code 时模型跑的是**官方推荐采样、随机种子**（实测 `/slots`：temp 1.0 / top_p 1.0 /
  top_k 20 / seed -1）。**2026-08-02 起裸测也改成按各模型官方推荐采样**（不再固定温度 0），
  两套的采样条件因此对齐了；但裸测每题跑 3 次取「过几次/3」，本套仍按各用例自己的 k。
...
T2 反问 ✓ 19.6s／R4 修对 17/17 ✓ 150s／E1I 闭卷命中 5/7、没甩锅、全文已存待裁判。
...
- **C1 题面指定的文件名曾和项目里已有的模块重名**
  → 模型把功能加进已有模块、判分器只认项目根，判成 F2P 0/2。**这是题面歧义不是模型答错**，
  2026-08-02 改成项目里不存在的 `count_readings.py`。**给确定文件名的题，先查这名字重不重名。**
...
- **`suites/ops_scripts/notify.py` 里的 webhook / 令牌 / 手机号全是编的**，
  ...
  写了就等于剧透题目。
- **`suites/gw7_field/` 里的设备、固件版本、故障机制全是虚构的**，
